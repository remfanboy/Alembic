...
let parse_info ctx =
  let pat_simple = (function MINUS | EOF -> true | _ -> false) in
  let pat_fun ctx = match eat_exact_depth ctx pat_simple 3 with
    | Error _ -> false, ctx
    | Ok(ctx) -> true, ctx in
  match join_text_until_pat ctx pat_fun with
    | Error err -> Error err
    | Ok(text, ctx) ->
      match eat_exact_depth ctx pat_simple 3 with
      | Error err -> Error err
      | Ok(ctx) -> Ok(text, ctx)
